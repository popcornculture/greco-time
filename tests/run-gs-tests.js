...
/* Every key the rest of the code addresses must resolve. If a header is renamed to
 * match the real MyCase template, these still hold — that is the point of keys. */
...
eq(row[col('client')], 'Maria Ramirez', 'client lands in its column');
eq(row[col('timekeeper')], 'Paul Greco', 'timekeeper lands in its column');
eq(row[col('hours')], 1.5, 'hours land as a number, not a string');
eq(row[col('description')], 'Phone call w/ DA', 'description lands in its column');
